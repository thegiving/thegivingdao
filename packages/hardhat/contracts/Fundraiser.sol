...
contract Fundraiser is AccessControlEnumerable, Pausable {
    ...
    modifier campaignManagerOnly() {
        ...
        require(
            account.owner == _msgSender() || hasRole(ADMIN_ROLE, _msgSender()),
            "UNAUTHORIZED"
        );
        _;
    }
    ...
    function setCampaignCategories(string[] memory categories)
        public
        adminOnly
    {
    ...
    function _setCampaignCategory(bytes32 categoryId, string memory name)
        internal
    {
    ...
    function _campaignCategoryExists(bytes32 categoryId)
        internal
        view
        returns (bool)
    {
    ...
    function _validateCampaign(
        ...
        require(
            bytes(campaignCategories[categoryId]).length > 0,
            "CATEGORY_EMPTY"
        );
        ...
        if (
            account.kind == AccountKind.Charity &&
    ...
    function updateCampaign(
        ...
        require(
            account.owner != address(0) &&
                campaign.account.owner == account.owner,
            "ACCOUNT_NOT_FOUND"
        );
    ...
    function donate(bytes32 campaignId) public payable {
        ...
        require(campaigns[campaignId].id != 0, "CAMPAIGN_NOT_FOUND");
        require(
            campaigns[campaignId].state == CampaignState.Active,
            "CAMPAIGN_NOT_ACTIVE"
        );
    ...
    function distributeFunds(bytes32 campaignId, uint256 amount)
        public
        accountOwnerOnly
    {
    ...
    function updateAccount(string calldata dataCID, AccountKind kind)
        public
        accountOwnerOnly
    {
        Account storage account = accounts[_msgSender()];
    ...
    function getCampaignKindByValue(CampaignKind _kind)
        external
        pure
        returns (string memory)
    {
        ...
        if (CampaignKind.OptionalGovernance == _kind)
            return "Optional Governance";
        return "INVALID_TYPE";
    }

    function getAccountKindByValue(AccountKind _kind)
        external
        pure
        returns (string memory)
    {
